...
/datum/species/proc/handle_digestion(mob/living/carbon/human/H)
	...
	if(HAS_TRAIT(H, TRAIT_YUANITE) && H.bloodpool > 0 && H.stat != DEAD && !HAS_TRAIT(H, TRAIT_NOHUNGER))
		var/vitae_rate = HUNGER_FACTOR
		H.bloodpool = max(0, H.bloodpool - vitae_rate)
...
/datum/species/proc/get_vitae_stage(mob/living/carbon/human/H)
	if(!HAS_TRAIT(H, TRAIT_YUANITE))
		return 0

	switch(H.bloodpool)
		if(VITAE_LEVEL_HYDRATED to INFINITY)
			return -1
		if(VITAE_LEVEL_SMALLTHIRST to VITAE_LEVEL_HYDRATED)
			return 0
		if(VITAE_LEVEL_THIRSTY to VITAE_LEVEL_SMALLTHIRST)
			return 1
		if(VITAE_LEVEL_DEHYDRATED to VITAE_LEVEL_THIRSTY)
			return 2
		if(0 to VITAE_LEVEL_DEHYDRATED)
			return 3
	return 0
